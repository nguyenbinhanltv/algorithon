...
describe("LanguagesManager class.", function () {
  it("getLanguagesTable() should return a map of languagesTable.", function () {
    ...
    expect(returnedLanguagesMap.entries()).to.be.eql(
      constLanguagesMap.entries()
    );
  });
  ...
  it("isLangSupported() should return true only for supported language & version.", function () {
    ...
    // ACT
    const result01 = LanguagesManager.isLangSupported(
      lang01.langId,
      lang01.version
    );
    const result02 = LanguagesManager.isLangSupported(
      lang02.langId,
      lang02.version
    );
    const result03 = LanguagesManager.isLangSupported(
      lang03.langId,
      lang03.version
    );
    // ASSERT
  ...
  it("getLanguageVersionIndex(lang, version) should return the version index for supported language & version.", function () {
    ...
    // ACT
    const result01 = LanguagesManager.getLanguageVersionIndex(
      lang01.langId,
      lang01.version
    );
    const result02 = LanguagesManager.getLanguageVersionIndex(
      lang02.langId,
      lang02.version
    );
    const result03 = LanguagesManager.getLanguageVersionIndex(
      lang03.langId,
      lang03.version
    );
    // ASSERT
